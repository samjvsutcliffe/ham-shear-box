  ...
  (let* ((sim (cl-mpm/setup:make-simple-sim
    ...
    (progn
      (let* ((E *elastic-constant*)
             ...
             (init-stress (cl-mpm/damage::mohr-coloumb-coheasion-to-tensile 131d3 42d0))
             ;(gf 5d0)
             (gf 4.8d0)
             ;; (gf 4.8d0)
             (length-scale 7.5d-3)
...
(defun mpi-loop ()
  (let* ((refine (if (uiop:getenv "REFINE") (parse-integer (uiop:getenv "REFINE")) 2))
         ...
         (output-dir (format nil "/nobackup/rmvn14/paper-1/plastic-damage/output-~F_0.5_~D_~f_~f_~F-~f/" refine mps scale damage overscale load))
         )
    (setf *damage* damage)
