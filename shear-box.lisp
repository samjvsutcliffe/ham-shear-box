...
(defun mpi-loop ()
  (let* ((refine (if (uiop:getenv "REFINE") (parse-integer (uiop:getenv "REFINE")) 2))
         ...
         (epsilon-scale 1d4)
         (piston-scale 1d0)
         (output-dir (format nil "/nobackup/rmvn14/paper-1/damage-se/output-~F_~D_~f_~f_~F-~f/" refine mps scale piston-scale epsilon-scale load))
         )
    (setf *damage* damage)
